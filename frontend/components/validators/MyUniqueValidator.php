 ...
 * @since 2.0
 */
class MyUniqueValidator extends Validator
{
    /**
    ...
    public function validateAttribute($model, $attribute)
    {
        ...
        if ($this->modelExists($targetClass, $conditions, $model)) {
            if (is_array($targetAttribute) && count($targetAttribute) > 1) {
                ...
                $id = $this->getExistedId($targetClass, $targetAttribute, $model);
                $this->addError($model, $attribute, $this->message . " $id");
            }
        }
    }

    private function getExistedId($targetClass, $targetAttribute, $model)
    {
        return $targetClass::find()
                ->select('id')
                ->andWhere([$targetAttribute => $model->$targetAttribute])
                ->limit(1)                
                ->scalar();
    }
    /**
