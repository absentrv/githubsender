...
class Request extends \yii\web\Request {
    ...
    /**
     * Get Current langugage Url from localesUrls array
     * @return type
     */
    ...
    public function getLangUrl() {
        ...
        $this->langUrl = $this->getUrl(); //полный URL
       
        $url_list = explode('/', $this->getUrl());        
        if (!empty($url_list[1])) {            
            $lang_url = $url_list[1];                  
            if (in_array($lang_url, array_values(Yii::$app->params['localesUrls']))) {
                Yii::$app->language = array_search($lang_url, Yii::$app->params['localesUrls']);
            } 
            //Удалить метку языка из URL           
            if ($lang_url === self::getLangApp()) {
                $url = preg_replace("/^\/$lang_url/", '', $this->langUrl);                
                return $url;
            }
